...
const TONGCHENG_CODE_URL =
  "https://www.ly.com/trainbffapi/getCodeByStationName";
...
export const GET_HUOCHEPIAO_LIST: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      depStationName: {
        type: "string",
        description:
          "出发火车站名称或者出发城市名称。支持输入火车站全称（如北京西站）或城市名（如北京）",
      },
      arrStationName: {
        type: "string",
        description:
          "目的地火车站名称或者到城市名称。支持输入火车站全称（如北京西站）或城市名（如北京）",
      },
      depDate: {
        type: "string",
...
export const SUPPORT_TOOLS = [GET_SYSTIME] as const;
...
export async function handleGetHuochepiaoList(
  ...
  if (
    ...
    return buildErrorResponse(
      "参数不完整，请确保提供了depStationName、arrStationName、depDate"
    );
  }
  ...
  const finalResult = convertFieldNames(
    resultData,
    depStationName,
    arrStationName
  );
...
async function handleGetSystemTime(): Promise<any> {
  // 获取当前系统时间
  const now = new Date();
    ...
    车次列表:
      resultData.trains?.map(
        (item: {
          ...
          出发火车站是否是经停站:
            item.depStationCode === item.startStationCode
              ? "不是经停站"
              : "是经停站",
          出发火车站是否是始发站:
            item.depStationCode === item.startStationCode
          ...
          历时小时分钟: item.runTime,
          座位类型: item.trainAvs?.map(
            (seat: { seatClassCode: any; price: any; num: any }) => ({
...
const server = new McpServer({
  ...
  instructions:
    "本 MCP 服务专为12306火车票查询设计，提供车次列表查询和当前系统时间查询功能。服务功能和规则如下",
});

server.registerTool(
  GET_HUOCHEPIAO_LIST.name,
  {
    description: GET_HUOCHEPIAO_LIST.description!!,
    inputSchema: {
      depStationName: z.string().describe("出发火车站名称或者出发城市名称"),
      arrStationName: z.string().describe("目的地火车站名称或者到城市名称"),
      depDate: z.string().describe("出发日期：格式必须为yyyy-MM-dd"),
    },
  },
  async ({ depStationName, arrStationName, depDate }) => {
    // const { depStationName, arrStationName, depDate } = request.value;
    return await handleGetHuochepiaoList(
      depStationName,
      arrStationName,
      depDate
    );
