...
exports.getCreateActivityTool = getCreateActivityTool;
...
function getCreateActivityTool() {
    return {
        name: "创建一个新的 Android Activity 组件",
        description: "根据指定的包名和组件名称，生成一个包含 Activity、ViewModel 和 XML Layout 的完整代码模板，符合 MathWallet Android 项目的结构和规范。",
        ...
        handler: async (extra) => {
            ...
            const activityPath = `app/src/main/java/${pkg.replace(/\./g, "/")}/${activityClass}.kt`;
            const viewModelPath = `app/src/main/java/${pkg.replace(/\./g, "/")}/${viewModelClass}.kt`;
            const layoutPath = `app/src/main/res/layout/${layoutName}.xml`;
            const tpl = exports.DEFAULT_TEMPLATES;
            // Binding class and import, e.g. ActivityTestOneBinding
            const bindingClass = `Activity${comp}Binding`;
            const bindingPackage = (extra && extra.bindingPackage) || "com.mgx.mathwallet.databinding";
...
//# sourceMappingURL=math_create_file.js.map